...
warnings.filterwarnings(
    "ignore", message="No libpcap provider available ! pcap won't be used"
)
...
def extract_packet_data(packets):
    ...
    for packet in tqdm(packets, desc="Processing packets", unit="packet"):
        if IP in packet:
            ...
            size = len(packet)
            packet_data.append(
                {"src_ip": src_ip, "dst_ip": dst_ip, "protocol": protocol, "size": size}
            )
...
def analyze_packet_data(df):
    ...
    ip_communication = (
        df.groupby(["src_ip", "dst_ip"]).size().sort_values(ascending=False)
    )
    ip_communication_percentage = ip_communication / ip_communication.sum() * 100
    ip_communication_table = pd.concat(
        [ip_communication, ip_communication_percentage], axis=1
    ).reset_index()
    ...
    protocol_counts_df = pd.concat(
        [protocol_frequency, protocol_counts], axis=1
    ).reset_index()
    ...
    ip_communication_protocols["Protocol"] = ip_communication_protocols[
        "Protocol"
    ].apply(protocol_name)
...
def extract_packet_data_security(packets):
    ...
    for packet in tqdm(
        packets, desc="Processing packets for port scanning activity", unit="packet"
    ):
        if IP in packet:
            src_ip = packet[IP].src
            dst_ip = packet[IP].dst
...
def detect_port_scanning(df, port_scan_threshold):
    ...
    unique_ports_per_ip = (
        port_scan_df.groupby("src_ip").size().reset_index(name="unique_ports")
    )

    # Check for a large number of packets to different ports on a single IP address
    potential_port_scanners = unique_ports_per_ip[
        unique_ports_per_ip["unique_ports"] >= port_scan_threshold
    ]
    ip_addresses = potential_port_scanners["src_ip"].unique()

    if len(ip_addresses) > 0:
        logger.warning(
            f"Potential port scanning detected from IP addresses: {', '.join(ip_addresses)}"
        )
...
def print_results(
    ...
    logger.info(
        tabulate(
            ip_communication_table,
            headers=["Source IP", "Destination IP", "Count", "Percentage"],
            tablefmt="grid",
            floatfmt=".2f",
        )
    )
    ...
    logger.info(
        tabulate(
            ip_communication_protocols,
            headers=["Source IP", "Destination IP", "Protocol", "Count", "Percentage"],
            tablefmt="grid",
            floatfmt=".2f",
        )
    )
...
if __name__ == "__main__":
    # Directly specify the PCAP file path and port scan threshold here
    pcap_file = (
        "path_to_your_pcap_file.pcap"  # Change this to your actual PCAP file path
    )
    port_scan_threshold = 100  # Set your desired port scan threshold
